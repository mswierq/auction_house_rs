...
impl Engine {
    ...
    pub fn refresh_token(&self, token_str: &str) -> Result<String, Box<dyn std::error::Error>> {
...
mod test {
    ...
    fn test_refresh_token() {
        let engine = Engine::new();
        let token_str = engine.create_new_token("user").unwrap();
        let new_token_str = engine.refresh_token(&token_str).unwrap();
        let token: TokenType<_> = token_str.verify_with_key(&engine.key).unwrap();
        assert_eq!(token.claims().get("user").unwrap(), "user");
        let new_token: TokenType<_> = new_token_str.verify_with_key(&engine.key).unwrap();
        assert_eq!(new_token.claims().get("user").unwrap(), "user");
    }
